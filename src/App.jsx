...
export default function App() {
  ...
  const getCropName = (vegIdOrObj) => {
    if (!vegIdOrObj) return '';
    if (typeof vegIdOrObj === 'object' && vegIdOrObj.name) return vegIdOrObj.name;
    const idStr = typeof vegIdOrObj === 'object' ? vegIdOrObj._id : vegIdOrObj;
    const match = vegetables.find(v => v._id === idStr);
  ...
  const [expenseCategory, setExpenseCategory] = useState('Crop Investment');
  const [expenseVegId, setExpenseVegId] = useState('');
  ...
  useEffect(() => {
    if (showCollectBillCash) {
      ...
      const commAmt = Math.round(((gross * commRateNum) / 100) * 100) / 100;
      const totalDeductions = Math.round((commAmt + (Number(collectLabor) || 0) + (Number(collectTax) || 0) + (Number(collectOther) || 0)) * 100) / 100;
      const remainingNet = Math.round(Math.max(0, gross - totalDeductions) * 100) / 100;
        ...
        <div className="auth-container">
          <div className="auth-card">
            ...
            <form onSubmit={handleAuthSubmit}>
              ...
              {authError && (
                <div style={{ color: '#C62828', backgroundColor: '#FFEBEE', padding: '10px', borderRadius: '8px', marginTop: '16px', fontSize: '0.9rem' }}>
                  {authError}
                </div>
              )}
            </form>
  ...
  const getGroupedSales = () => {
    ...
    return Object.values(groups).map(g => {
      ...
      return {
        ...
        totalPaid: Math.round(g.totalPaid * 100) / 100,
        totalWeight: Math.round(g.totalWeight * 100) / 100,
        totalCashReceived: Math.round(cashReceived * 100) / 100,
  ...
  return (
    <div className="app-container">
              ...
              {groupedSalesList.map(g => (
                <div key={g.key} className="item-card" style={{ padding: '12px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', fontSize: '0.85rem', color: 'var(--text-secondary)', borderTop: '1px solid var(--border-color)', paddingTop: '8px' }}>
                    ...
                    <span>
                      Collected: {g.totalDeductions > 0 ? (
                        <>₹{g.totalCashReceived} / {g.totalCashReceived}+{g.totalDeductions}</>
                      ) : (
                        <>₹{g.totalPaid}</>
                      )} / <strong>₹{g.totalNet}</strong>
                    </span>
                  </div>
      ...
      {activeTab === 'expenses' && (
        <div className="dashboard-content">
          <div className="section-title">
            <span>Expenses & Investment (लागत और खर्चा)</span>
            <button 
              className="add-link" 
              onClick={() => {
                setExpenseCategory('Crop Investment');
                setExpenseVegId(vegetables[0]?._id || '');
                setExpenseTitle('');
                setExpenseAmount('');
                setExpenseNote('');
          ...
          <div className="stats-grid">
            <div className="stat-card">
              <div className="stat-label">Crop Investment (फसल लागत)</div>
              <div className="stat-val earnings" style={{ color: 'var(--primary-color)' }}>
                ₹{(Math.round(expenses.filter(e => e.category === 'Crop Investment').reduce((sum, e) => sum + (e.amount || 0), 0) * 100) / 100).toLocaleString('en-IN')}
              </div>
            </div>
            <div className="stat-card">
              <div className="stat-label">Personal Expense (व्यक्तिगत खर्चा)</div>
              <div className="stat-val outstanding" style={{ color: '#E53935' }}>
                ₹{(Math.round(expenses.filter(e => e.category === 'Personal Expense').reduce((sum, e) => sum + (e.amount || 0), 0) * 100) / 100).toLocaleString('en-IN')}
              </div>
            </div>
          </div>

          {/* Crop Profit/Loss analysis section */}
          <div className="section-title" style={{ marginTop: '24px', marginBottom: '12px' }}>
            <span>Crop Profit & Loss (फसल मुनाफा/नुकसान)</span>
          </div>

          {vegetables.length === 0 ? (
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>No crop profiles registered yet. Add crops on the Dashboard tab.</p>
          ) : (
            <div className="list-container" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
              {vegetables.map(v => {
                const cropSales = Math.round(sales.filter(s => s.vegetableName === v.name).reduce((sum, s) => sum + (s.netAmount || 0), 0) * 100) / 100;
                const cropExpenses = Math.round(expenses.filter(e => e.category === 'Crop Investment' && (e.vegetableId?._id === v._id || e.vegetableId === v._id)).reduce((sum, e) => sum + (e.amount || 0), 0) * 100) / 100;
                const cropPersonalExpenses = Math.round(expenses.filter(e => e.category === 'Personal Expense' && (e.vegetableId?._id === v._id || e.vegetableId === v._id)).reduce((sum, e) => sum + (e.amount || 0), 0) * 100) / 100;
                const netProfit = Math.round((cropSales - cropExpenses - cropPersonalExpenses) * 100) / 100;
                
                return (
                  <div key={v._id} className="item-card" style={{ borderLeft: `4px solid ${netProfit >= 0 ? 'var(--primary-color)' : 'var(--danger-text)'}` }}>
                    <div className="item-header">
                      <strong>{v.name}</strong>
                      <span className="badge" style={{
                        backgroundColor: netProfit >= 0 ? 'var(--success-bg)' : 'var(--danger-bg)',
                        color: netProfit >= 0 ? 'var(--success-text)' : 'var(--danger-text)'
                      }}>
                        {netProfit >= 0 ? 'Profit' : 'Loss'}
                      </span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '8px', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>Total Sales: ₹{cropSales.toLocaleString('en-IN')}</span>
                        <span>Total Cost: ₹{cropExpenses.toLocaleString('en-IN')}</span>
                      </div>
                      {cropPersonalExpenses > 0 && (
                        <div style={{ display: 'flex', justifyContent: 'space-between', color: '#E53935' }}>
                          <span>Personal Spend:</span>
                          <span>-₹{cropPersonalExpenses.toLocaleString('en-IN')}</span>
                        </div>
                      )}
                    </div>
                    <div style={{ borderTop: '1px dashed var(--border-color)', marginTop: '8px', paddingTop: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontWeight: 600, fontSize: '0.9rem' }}>Net Return:</span>
                      <strong style={{ color: netProfit >= 0 ? 'var(--success-text)' : 'var(--danger-text)', fontSize: '1rem' }}>
                  ...
                  <div className="item-header">
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginTop: '4px' }}>
                        <span className="badge" style={{
                          backgroundColor: e.category === 'Crop Investment' ? 'var(--success-bg)' : '#ECEFF1',
                          color: e.category === 'Crop Investment' ? 'var(--success-text)' : '#546E7A',
                          fontSize: '0.7rem',
                          padding: '2px 6px'
                        }}>
                          {e.category === 'Crop Investment' ? `Crop: ${getCropName(e.vegetableId)}` : (getCropName(e.vegetableId) ? `Personal (Crop: ${getCropName(e.vegetableId)})` : 'Personal')}
                        </span>
                      </div>
                    </div>
              ...
              {showCollectBillCash && (
                <div className="item-card" style={{ marginBottom: '16px', border: '1px dashed var(--primary-color)' }}>
                  ...
                  <div className="ledger-stat">
                    ...
                    <span>-₹{(Math.round((((showCollectBillCash.totalNet - showCollectBillCash.totalPaid) * (Number(collectCommRate) || 0)) / 100) * 100) / 100).toLocaleString('en-IN')}</span>
                  </div>
                  <div className="ledger-stat">
      ...
      {showAddExpense && (
        <div className="modal-overlay" onClick={() => setShowAddExpense(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">Record Expense / Investment</span>
              <button className="close-btn" onClick={() => setShowAddExpense(false)}><X /></button>
            </div>
            
            <form onSubmit={async (e) => {
              e.preventDefault();
              const cat = expenseCategory;
              const vegId = cat === 'Crop Investment' ? (expenseVegId || (vegetables[0]?._id || '')) : expenseVegId;
              ...
              <div className="form-group">
                <label>Select Crop (फसल चुनें){expenseCategory === 'Crop Investment' ? '*' : ' (Optional)'}</label>
                <select 
                  className="input-field" 
                  value={expenseVegId} 
                  onChange={e => setExpenseVegId(e.target.value)}
                  required={expenseCategory === 'Crop Investment'}
                >
                  {expenseCategory === 'Personal Expense' && (
                    <option value="">-- None / No Crop --</option>
                  )}
                  {vegetables.length === 0 ? (
                    <option value="">-- No Vegetables Added Yet --</option>
                  ) : (
              ...
              <div className="form-group">
                ...
                <input 
                  type="text" 
                  className="input-field" 
                  placeholder={expenseCategory === 'Crop Investment' ? "e.g. Seeds, Davai, Fertilizer" : "e.g. Pohe, Mobile Recharge, Movie"} 
                  value={expenseTitle}
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  value={expenseAmount} 
                  inputMode="decimal"
                  onChange={handleNumericChange(setExpenseAmount)} 
                  placeholder="e.g. 500"
                  required 
                />
              </div>
